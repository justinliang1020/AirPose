...
PoseVRDriver::TrackingReferenceDevice::TrackingReferenceDevice(std::string serial):
    serial_(serial)
{
...
std::string PoseVRDriver::TrackingReferenceDevice::GetSerial()
{
    return this->serial_;
}

void PoseVRDriver::TrackingReferenceDevice::Update()
{
...
DeviceType PoseVRDriver::TrackingReferenceDevice::GetDeviceType()
{
    return DeviceType::TRACKING_REFERENCE;
}

vr::TrackedDeviceIndex_t PoseVRDriver::TrackingReferenceDevice::GetDeviceIndex()
{
    return this->device_index_;
}

vr::EVRInitError PoseVRDriver::TrackingReferenceDevice::Activate(uint32_t unObjectId)
{
...
void PoseVRDriver::TrackingReferenceDevice::Deactivate()
{
    this->device_index_ = vr::k_unTrackedDeviceIndexInvalid;
}

void PoseVRDriver::TrackingReferenceDevice::EnterStandby()
{
}

void* PoseVRDriver::TrackingReferenceDevice::GetComponent(const char* pchComponentNameAndVersion)
{
    return nullptr;
}

void PoseVRDriver::TrackingReferenceDevice::DebugRequest(const char* pchRequest, char* pchResponseBuffer, uint32_t unResponseBufferSize)
{
    if (unResponseBufferSize >= 1)
        pchResponseBuffer[0] = 0;
}

vr::DriverPose_t PoseVRDriver::TrackingReferenceDevice::GetPose()
{
    return last_pose_;
}
